...
export default function AlertsPage() {
  ...
  const teacherAlerts: TeacherAlert[] = useMemo(() => {
    ...
    return alerts;
  }, [timetable, attendanceData, date, threshold, teachers]);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20 lg:pb-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Bell className="w-6 h-6 text-yellow-500" />Alert System</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm">Teacher late alerts & parent notification queue</p>
      </motion.div>

      {/* controls */}
      <div className={S.card}>
        <h3 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><Zap className="w-4 h-4 text-primary-600" />Alert Controls</h3>
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div><label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Late Threshold (min)</label>
      ...
      <div className={cn(S.card, "p-0 overflow-hidden")}>
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-500" />Teacher Late Alerts</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Teachers who haven't marked attendance after grace period</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-gray-50 dark:bg-gray-800">
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Teacher</th>
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Class</th>
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Subject</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Period</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Time</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {teacherAlerts.length ? teacherAlerts.map((a, i) => (
                <motion.tr key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="px-3 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{a.teacher}</td>
                  <td className="px-3 py-3 text-sm text-gray-600 dark:text-gray-300">{a.class}</td>
                  <td className="px-3 py-3 text-sm text-gray-600 dark:text-gray-300">{a.subject}</td>
                  <td className="px-3 py-3 text-center text-sm text-gray-700 dark:text-gray-300">{a.period}</td>
                  <td className="px-3 py-3 text-center text-sm text-gray-500 dark:text-gray-400">{a.time}</td>
      ...
      <div className={cn(S.card, "p-0 overflow-hidden")}>
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2"><Users className="w-4 h-4 text-primary-600" />Parent Notification Queue</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Auto-generated when a student is marked absent</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-gray-50 dark:bg-gray-800">
              ...
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Class</th>
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Subject</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Date</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Period</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Status</th>
      ...
      {teachers.length === 0 && (
        <div className="text-center text-xs text-gray-400 dark:text-gray-500">Tip: Add teachers and timetable in Admin Panel to enable late alerts.</div>
      )}
    </div>
  );
}
