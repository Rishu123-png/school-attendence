...
export default function SchedulePage() {
  ...
  const slots = timetable
    .filter((s) =>
      s.day?.toLowerCase() === selectedDay &&
      (!selectedClass || s.classId === selectedClass || s.classLabel === selectedClass || s.class === selectedClass)
    )
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20 lg:pb-0">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className={S.card}>
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => dayIdx > 0 && setSelectedDay(days[dayIdx - 1])} disabled={dayIdx <= 0} className={cn(S.btnGhost, "p-2")}>
            <ChevronLeft className="w-5 h-5" />
          </button>

          <div className="text-center">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white capitalize">{selectedDay}</h2>
            <p className="text-xs text-gray-400 dark:text-gray-500">{slots.length} periods</p>
          </div>

          <button onClick={() => dayIdx < days.length - 1 && setSelectedDay(days[dayIdx + 1])} disabled={dayIdx >= days.length - 1} className={cn(S.btnGhost, "p-2")}>
        ...
        {slots.length ? slots.map((slot, i) => (
          ...
          >
            <div className="flex flex-col items-center min-w-[84px]">
              <span className="text-xs text-gray-400 dark:text-gray-500">{slot.period}</span>
              <span className="text-sm font-bold text-gray-900 dark:text-white">{slot.startTime || slot.time}</span>
              {slot.endTime && <span className="text-[10px] text-gray-400 dark:text-gray-500">to {slot.endTime}</span>}
            </div>

            <div className={cn("px-3 py-2 rounded-lg border text-sm font-semibold", pColor(slot.period))}>
              {slot.period}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {slot.classLabel || [slot.class, slot.section].filter(Boolean).join(" - ") || "—"}
                </span>
