...
export default function TopBunkersPage() {
  ...
  const bunks: BunkRow[] = useMemo(() => {
    ...
    // build map: studentId+subject -> { count, lastDate, lastPeriod }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20 lg:pb-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">🕵️ Top Bunkers</h1>
        <p className="text-gray-500 text-sm">Class-wise & subject-wise bunk summary</p>
      </motion.div>

      {/* stats */}
      <div className="grid grid-cols-3 gap-3">
        <div className={cn(S.card, "py-3 text-center bg-gradient-to-br from-red-50 to-white")}><p className="text-xs text-red-600">Total Bunk Events</p><p className="text-2xl font-bold text-red-700">{totalBunks}</p></div>
        <div className={cn(S.card, "py-3 text-center bg-gradient-to-br from-orange-50 to-white")}><p className="text-xs text-orange-600">Unique Students</p><p className="text-2xl font-bold text-orange-700">{uniqueStudents}</p></div>
        <div className={cn(S.card, "py-3 text-center bg-gradient-to-br from-yellow-50 to-white")}><p className="text-xs text-yellow-600">Repeat Offenders (≥3)</p><p className="text-2xl font-bold text-yellow-700">{topBunkers}</p></div>
      </div>
      ...
      <div className={cn(S.card, "p-0 overflow-hidden")}>
        <div className="p-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-900 flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-500" />Repeated Subject Bunks</h3>
          <p className="text-xs text-gray-400 mt-0.5">Sorted by bunk count — highest first</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-gray-50">
              ...
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500">Class</th>
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500">Subject</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-red-600">Bunks</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500">Last Date</th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500">Last Period</th>
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {bunks.length ? bunks.map((b, i) => (
                <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.01 }} className="hover:bg-gray-50">
                  <td className="px-3 py-3 text-sm text-gray-500">{i + 1}</td>
                  <td className="px-3 py-3"><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-100 to-red-200 text-red-700 flex items-center justify-center text-xs font-bold">{b.rollNo}</div><span className="text-sm font-medium text-gray-900">{b.name}</span></div></td>
                  <td className="px-3 py-3 text-center text-xs text-gray-600">{b.class}{b.section ? ` · ${b.section}` : ""}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{b.subject}</td>
                  <td className="px-3 py-3 text-center">
                    <span className={cn("text-sm font-bold px-2.5 py-0.5 rounded-full", b.count >= 5 ? "bg-red-100 text-red-700" : b.count >= 3 ? "bg-orange-100 text-orange-700" : "bg-yellow-100 text-yellow-700")}>{b.count}</span>
                  </td>
                  <td className="px-3 py-3 text-center text-xs text-gray-500">{new Date(b.lastDate).toLocaleDateString()}</td>
                  <td className="px-3 py-3 text-center text-xs text-gray-500">{b.lastPeriod}</td>
                </motion.tr>
              )) : <tr><td colSpan={7} className="text-center py-12 text-gray-400"><Eye className="w-10 h-10 mx-auto mb-2 opacity-40" /><p className="text-sm">🎉 No bunk events found!</p><p className="text-xs mt-1">All students attending or no absences recorded</p></td></tr>}
            </tbody>
          </table>
        </div>
      </div>
