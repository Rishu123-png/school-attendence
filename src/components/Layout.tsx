...
import { Home, GraduationCap, ClipboardCheck, BarChart3, Bell, Menu, LogOut, User, ChevronDown, Sparkles, BookOpen, CalendarDays, Megaphone, Users, Copy, Check, FileText, Trophy, UserX, PartyPopper, UserSearch, X } from "lucide-react";
...
export default function Layout() {
  ...
  const notifStyle = (kind: string) => {
    switch (kind) {
      case "late-alert": return { dot: "bg-red-500", bg: "bg-red-50/40" };
      case "reminder":   return { dot: "bg-primary-500", bg: "bg-primary-50/40" };
      case "announcement": return { dot: "bg-accent-500", bg: "bg-accent-50/40" };
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* ── Sidebar ─────────────────── */}
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-100 shadow-sm transform transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto",
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-3 px-6 py-5 border-b border-gray-100">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">School OS</h1>
                      ...
                      return (
                        <button key={item.path} onClick={() => { navigate(item.path); setSidebar(false); }}
                          className={cn("flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm font-medium transition-all",
                            active ? "bg-gradient-to-r from-primary-50 to-primary-100/50 text-primary-700 shadow-sm" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900")}>
          ...
          {schoolId && (
            <div className="px-4 py-3 border-t border-gray-100">
              ...
              <button onClick={copySchoolCode}
                className="flex items-center gap-2 w-full px-3 py-1.5 rounded-lg bg-gray-50 hover:bg-gray-100 text-xs font-mono text-gray-700 transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-gray-100 shadow-sm">
          <div className="flex items-center justify-between px-4 lg:px-6 h-14">
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebar(true)} className="p-2 rounded-xl text-gray-600 hover:bg-gray-100 lg:hidden"><Menu className="w-5 h-5" /></button>
              <span className="hidden sm:block text-sm font-medium text-gray-700">{currentLabel}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="relative">
                <button onClick={() => { setNotifOpen(!notifOpen); setProfileOpen(false); if (!notifOpen) markAllRead(); }} className="relative p-2 rounded-xl text-gray-600 hover:bg-gray-100">
                ...
                <AnimatePresence>
                  {notifOpen && (
                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }}
                      className="absolute right-0 mt-2 w-[340px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                      <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                        <h3 className="font-semibold text-gray-900 flex items-center gap-2"><Bell className="w-4 h-4 text-primary-600" />Notifications</h3>
                        ...
                        {notifications.length ? notifications.map((n) => {
                          ...
                          return (
                            <div key={n.id} className={cn("flex items-start gap-3 px-4 py-3 transition-colors", !n.read && st.bg)}>
                              ...
                              <button onClick={() => dismiss(n.id)} className="p-1 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 shrink-0"><X className="w-3.5 h-3.5" /></button>
                            </div>
                          );
                        }) : (
                      ...
                      <div className="p-2 border-t border-gray-100">
                        <button onClick={() => { setNotifOpen(false); navigate("/alerts"); }} className="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-xl text-sm text-primary-600 hover:bg-primary-50 font-medium">Open Alert Center</button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="relative">
                <button onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false); }} className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-gray-100">
                ...
                <AnimatePresence>
                  {profileOpen && (
                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }}
                      ...
                      <div className="p-2">
                        <button className="flex items-center gap-3 w-full px-3 py-2 rounded-xl text-sm text-gray-700 hover:bg-gray-50"><User className="w-4 h-4" />Profile</button>
                        <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2 rounded-xl text-sm text-red-600 hover:bg-red-50"><LogOut className="w-4 h-4" />Sign Out</button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        {/* mobile bottom nav */}
        <nav className="sticky bottom-0 z-30 bg-white/90 backdrop-blur-lg border-t border-gray-100 lg:hidden">
            ...
            {MOBILE_NAV.map((item) => {
              ...
              return (
                <button key={item.path} onClick={() => navigate(item.path)}
                  className={cn("flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-all", active ? "text-primary-600" : "text-gray-400")}>
                  <div className={cn("p-1.5 rounded-lg", active && "bg-primary-50")}><Icon className="w-5 h-5" /></div>
