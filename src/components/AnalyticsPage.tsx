    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20 lg:pb-0">
        ...
        {[
          ...
          return (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 * i }}
              className={cn(S.card, `bg-gradient-to-br ${c.bg} to-white dark:to-gray-900`)}>
              <div className="mb-2"><div className="w-9 h-9 rounded-lg bg-white/80 dark:bg-gray-800 flex items-center justify-center shadow-sm"><Icon className="w-4 h-4 text-gray-700 dark:text-gray-300" /></div></div>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{c.val}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{c.label}</p>
              {c.bar !== undefined && <div className="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5"><div className="bg-primary-500 h-1.5 rounded-full" style={{ width: `${c.bar}%` }} /></div>}
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={S.card}>
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-primary-600" />Attendance Trend</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className={S.card}>
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><BookOpen className="w-4 h-4 text-accent-600" />Subject Performance</h3>
          <ResponsiveContainer width="100%" height={250}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-gray-50 dark:bg-gray-800">
              ...
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Performance</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {predictions.length ? predictions.slice(0, 15).map((p, i) => (
                <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="px-3 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{p.name}</td>
