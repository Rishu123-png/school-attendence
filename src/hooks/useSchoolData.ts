...
export interface SlotItem    { id: string; day: string; period: string; time: string; class: string; subject: string; teacher: string; room: string; type?: string }
...
export function useSchoolData() {
  ...
  useEffect(() => {
    ...
    unsubs.push(onValue(ref(db, `${root}/students`), (snap) => {
      const d = snap.val();
      setStudents(d ? Object.entries(d).map(([id, v]: [string, any]) => ({ id, name: v.name ?? "—", rollNo: v.rollNo ?? 0, class: v.class ?? "", section: v.section, email: v.email, phone: v.phone, parentName: v.parentName, parentPhone: v.parentPhone })).sort((a, b) => a.rollNo - b.rollNo) : []);
    }));

    unsubs.push(onValue(ref(db, `${root}/timetables`), (snap) => {
      const d = snap.val();
      setTimetable(d ? Object.entries(d).map(([id, v]: [string, any]) => ({ id, day: v.day ?? "", period: v.period ?? "", time: v.time ?? "", class: v.class ?? "", subject: v.subject ?? "", teacher: v.teacher ?? "", room: v.room ?? "", type: v.type })) : []);
    }));

    unsubs.push(onValue(ref(db, `${root}/teachers`), (snap) => {
      const d = snap.val();
      setTeachers(d ? Object.entries(d).map(([id, v]: [string, any]) => ({ id, name: v.name ?? "", email: (v.email ?? "").toLowerCase(), phone: v.phone, status: v.status ?? "invited", assignedClasses: v.assignedClasses ?? [], assignedSubjects: v.assignedSubjects ?? [] })) : []);
      setLoading(false);
    }));
  ...
  /* ── current teacher's assignment (filtered scope) ── */
  ...
  /* ── filter classes & subjects based on role/assignments ── */
