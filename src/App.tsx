      ...
      <div className="w-full max-w-md text-center space-y-5">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Your account <span className="font-medium text-gray-700 dark:text-gray-300">{profile?.email}</span> is registered, but your school admin has not added you to the teacher list yet.
          </p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 via-white to-accent-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 p-4">
      <div className="w-full max-w-md text-center space-y-5">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">
            For security, we sent a verification link to <span className="font-medium text-gray-700 dark:text-gray-300">{user?.email}</span>.
          </p>
        ...
        <div className="flex flex-col gap-3 max-w-xs mx-auto">
          <button onClick={() => window.location.reload()} className="w-full py-2.5 bg-primary-600 text-white rounded-xl font-medium shadow hover:bg-primary-700 transition-colors">
...
function ProtectedRoute() {
  ...
  if (!isSuperAdmin && profile && !schoolId) return <SchoolSetup />;
  if (!isAdmin && !isSuperAdmin && !currentTeacher) return <PendingApproval />;
  return <Outlet />;
}
