  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20 lg:pb-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-500 text-sm">Manage school holidays</p>
      </motion.div>

      {/* Add form (admin only) */}
      {isAdmin && (
        <div className={S.card}>
          <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2"><Sparkles className="w-4 h-4 text-primary-600" />Add Holiday</h3>
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div className="sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">Reason</label>
      ...
      <div className={cn(S.card, "p-0 overflow-hidden")}>
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <h3 className="font-semibold text-gray-900 flex items-center gap-2"><PartyPopper className="w-4 h-4 text-green-600" />Upcoming Holidays</h3>
        ...
        <div className="divide-y divide-gray-50">
          {upcoming.length ? upcoming.map((h, i) => (
            <motion.div key={h.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
              ...
              <div className="flex flex-col items-center justify-center w-14 h-14 rounded-xl bg-gradient-to-br from-primary-100 to-accent-100 shrink-0">
                <span className="text-[10px] font-medium text-gray-500 uppercase">{new Date(h.date).toLocaleDateString("en-US", { month: "short" })}</span>
                <span className="text-lg font-bold text-gray-900 leading-none">{new Date(h.date).getDate()}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-900">{typeIcon(h.type)} {h.reason}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className={cn(typeColor(h.type), "text-[10px] capitalize")}>{h.type}</span>
                  <span className="text-xs text-gray-400">{new Date(h.date).toLocaleDateString("en-US", { weekday: "long" })}</span>
                </div>
              </div>
              {isAdmin && <button onClick={() => del(h.id)} className="p-2 rounded-lg text-red-500 hover:bg-red-50"><Trash2 className="w-4 h-4" /></button>}
            </motion.div>
          )) : <div className="text-center py-12 text-gray-400"><CalendarDays className="w-10 h-10 mx-auto mb-2 opacity-40" /><p className="text-sm">No upcoming holidays</p></div>}
        </div>
      </div>
      ...
      {past.length > 0 && (
        <div className={cn(S.card, "p-0 overflow-hidden opacity-70")}>
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-semibold text-gray-900">Past Holidays</h3>
            <span className="text-xs text-gray-400">{past.length}</span>
          </div>
          <div className="divide-y divide-gray-50">
            {past.slice(-10).reverse().map((h) => (
              <div key={h.id} className="flex items-center gap-3 px-4 py-2.5">
                <span className="text-xs text-gray-400 w-24">{new Date(h.date).toLocaleDateString()}</span>
                <span className="text-sm text-gray-600 flex-1">{typeIcon(h.type)} {h.reason}</span>
                {isAdmin && <button onClick={() => del(h.id)} className="p-1.5 text-gray-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>}
              </div>
            ))}
          </div>
        </div>
