...
export default function MarksPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(ref(db, `schools/${schoolId}/attendance`), (snap) => {
      setAttendanceData(snap.val() ?? null);
    });
    return () => unsub();
  }, [schoolId]);
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto pb-20 lg:pb-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">📊 Marks</h1><p className="text-gray-500 dark:text-gray-400 text-sm">Record and track student performance</p></div>
        <button onClick={() => setShowAI(!showAI)} className={cn(S.btnSecondary, showAI && "bg-primary-50 border-primary-200 dark:bg-primary-900/20 dark:border-primary-700")}><Sparkles className="w-4 h-4" />AI Predictions</button>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className={cn(S.card, "bg-gradient-to-r from-primary-50 to-accent-50 border-primary-100/50 overflow-hidden dark:from-primary-900/10 dark:to-accent-900/10 dark:border-primary-800/30")}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.slice(0, 6).map((s) => {
                ...
                for (const date in attendanceData) {
                  for (const cls in attendanceData[date]) {
                    for (const period in attendanceData[date][cls]) {
                      const rec = attendanceData[date][cls][period][s.id];
                      if (!rec) continue;
              ...
              const p = predict(attDays, sm.map((m) => ({ pct: (m.score / m.maxScore) * 100 })));
              return (
                <div key={s.id} className="bg-white/80 dark:bg-gray-800/50 rounded-xl p-3 border border-white dark:border-gray-700">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{s.name}</p>
                    <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", p.performance.expected >= 80 ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : p.performance.expected >= 50 ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400")}>{p.performance.expected}%</span>
                  </div>
                  <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5"><div className={cn("h-1.5 rounded-full", p.performance.expected >= 80 ? "bg-green-500" : p.performance.expected >= 50 ? "bg-yellow-500" : "bg-red-500")} style={{ width: `${Math.min(100, p.performance.expected)}%` }} /></div>
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Confidence: {p.performance.confidence}%</p>
                </div>
              );
            })}
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className={S.card}>
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-20"><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Max</label>
            <input type="number" value={maxScore} onChange={(e) => setMaxScore(Number(e.target.value))} className={cn(S.input, "py-2")} /></div>
          <div className="relative flex-1 min-w-[130px]"><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Search</label>
